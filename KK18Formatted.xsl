...
<!-- 
     Bibliography and citation formatting in Word using the citation rules issued by the Kollegium Kalksburg.
	   Citation rules version: 21.04.2018
     
     Author(s): Johannes Berndorfer (johannesberndorfer82@gmail.com)
               
     Copyright: Copyright (c) 2018 Johannes Berndorfer

     Special thanks to "condingo" for the template of this stylesheet. Code @: https://raw.githubusercontent.com/codingo/BibWord/master/BibWord_Stylesheet.XSL
  -->
...
<xsl:stylesheet version="1.0"
  ...
  <!-- Variable containing all necessary data for a certain style of bibliography. Template used can be found @: https://raw.githubusercontent.com/codingo/BibWord/master/BibWord_Stylesheet.XSL -->
  <xsl:variable name="data">
    <general>
      <stylename>KK18 Formatted</stylename>
      <version>2018.04.21</version>
      <author>Johannes Berndorfer (johannesberndorfer82@gmail.com)</author>
      <description>Word implementation of the KK citation rules</description>
      <URL>https://www.kalksburg.at</URL>
      <comments>
        The KK citation rules are not created by me, find them under https://cloud.kalksburg.at
      </comments>
    ...
    <importantfields> 
      ...
      <source type="SoundRecording">
        <b:ImportantField>b:Composer/b:Composer/b:NameList</b:ImportantField>
        <b:ImportantField>b:Title</b:ImportantField>
    ...
    <citation>
      ...
      <source type="Book">
        <format>{%CitationPrefix%}{%Author:0|"[Anonym]"%}{, %Year%}{, %CitationPages:S. :S. %.}{%CitationSuffix%}</format>
      </source>
      <source type="InternetSite">
        <format>{%CitationPrefix%}{%Author:0|"[Anonym]"%}{, %CitationPages%}{, %DayAccessed%}{%CitationSuffix%}</format>
      </source>
    </citation>
    ...
    <footnotecitation>
      ...
      <source type="Book">
        <format>{%CitationPrefix%}{%Author:2|Editor:3|"[Anonymous]"%.}{ %Title%.{ %Edition:o% ed.}{ Vol %Volume%.}}{ %City%{ (%StateProvince%)}{: %Publisher%{; %Year%}}}{ %Publisher%{; %Year%}}{ %Year%}.{, %CitationPages:p. :pp. %}{%CitationSuffix%}</format>
      </source>
    </footnotecitation>
    ...
    <bibliography>
      ...
      <source type="Book">
        <column id="1">
          <halign>left</halign>
          <valign>top</valign>
          <format>{%Author:2|"[Anonym]"%: }{%Title%.{ %NumberVolumes% Bde.}{ %Edition%. Aufl.}}{ %City%{: %Publisher%{, %Year%{ (Bd. %Volume%)}.}}}</format>
        </column>
      </source>
      <source type="InternetSite">
        <column id="1">
          <halign>left</halign>
          <valign>top</valign>
          <format>{%Author:2|"[Anonym]"%: }{%Title%.}{ Online unter: %URL%}{ (zugegriffen am %DayAccessed%).}</format>
        </column>
      </source>
    </bibliography>
    ...
    <namelists>
      ...
      <list name="author" id="2">
        ...
        <multi_prefix></multi_prefix>
        <corporate>{%Corporate%}</corporate>
        <first_person>{%Last|First%}{, %First%}</first_person>
        <other_persons>{%Last|First%}{, %First%}</other_persons>
        <separator_between_if_two> / </separator_between_if_two>
        <separator_between_if_more_than_two> / </separator_between_if_more_than_two>
        <separator_before_last> / </separator_before_last>
        <max_number_of_persons_to_display>2</max_number_of_persons_to_display>
    ...
    <strings>
      <months>
        <month number="1">Jän</month>
        <month number="2">Feb</month>
        <month number="3">Mär</month>
